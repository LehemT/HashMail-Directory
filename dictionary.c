 ...
 * 
 * This file contains the implementation of the dictionary functions which add, retrieve, replace, 
 * and clear entries in the hash table. Operations include hashing keys, managing linked lists 
 * for hash collisions, and memory management for the dictionary entries.
 */
...
{    
    for (int i = 0; i < TABLESIZE; ++i) {
        ...
        // print NULL if the chain is empty
        if (dict[i] == NULL) {
            printf("NULL");
        }
        // move to the next line for the next slot
        printf("\n");
    }
}
...
_Bool replace(dict_t *dict, char *key, char *value)
{
    ...
    entry_t *existing_entry = search(*dict, key);
    if (existing_entry != NULL) {
        ...
        // free the old value & replace with the new
